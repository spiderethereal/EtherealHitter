          ...
          try {
              ...
              try {
                  if (ipStr !== "—") {
                      ...
                      const scamText = await scamRes.text();
                      if (scamText.trim().startsWith("<")) throw new Error("API returned HTML, not JSON");
                      const scamData = JSON.parse(scamText);
